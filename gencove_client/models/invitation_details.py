...
class InvitationDetails(BaseModel):
    ...
    last_status: Optional[InvitationStatus] = None
    expires: Optional[datetime] = None
    organization: InviteOrganization
    role: Annotated[str, Field(min_length=1, strict=True, max_length=255)]
    __properties: ClassVar[List[str]] = ["token", "last_status", "expires", "organization", "role"]
        ...
        _obj = cls.model_validate(
            {
                "token": obj.get("token"),
                "last_status": (
                    InvitationStatus.from_dict(obj["last_status"]) if obj.get("last_status") is not None else None
                ),
                "expires": obj.get("expires"),
                "organization": (
                    InviteOrganization.from_dict(obj["organization"]) if obj.get("organization") is not None else None
                ),
                "role": obj.get("role"),
            }
        )
