...
class SetUpMFA(BaseModel):
    ...
    secret: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(
        default=None, description='Authenticator secret, used when device_type is "generator"'
    )
    config_url: Optional[Annotated[str, Field(min_length=1, strict=True)]] = Field(
        default=None, description='Url to build the QR code image, used when device_type is "generator"'
    )
    __properties: ClassVar[List[str]] = ["device_type", "secret", "config_url"]

    @field_validator("device_type")
    def device_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["generator", "yubikey"]):
            raise ValueError("must be one of enum values ('generator', 'yubikey')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {"device_type": obj.get("device_type"), "secret": obj.get("secret"), "config_url": obj.get("config_url")}
        )
        return _obj
