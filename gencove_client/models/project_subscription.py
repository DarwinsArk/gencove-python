...
class ProjectSubscription(BaseModel):
    ...
    name: Optional[Annotated[str, Field(min_length=1, strict=True, max_length=255)]] = Field(
        default=None, description="User identifiable name, if left empty default name with a timestamp is used"
    )
    ...
    event_types: Optional[List[Optional[StrictStr]]] = Field(
        default=None, description="event types which will be delivered for current subscription"
    )
    has_secret: StrictBool = Field(description="Is webhook secret set")
    __properties: ClassVar[List[str]] = ["id", "name", "project", "target", "protocol", "event_types", "has_secret"]
